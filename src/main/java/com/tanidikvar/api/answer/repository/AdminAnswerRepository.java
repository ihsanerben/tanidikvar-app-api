...
public class AdminAnswerRepository {
 ...
 public List<AdminAnswer> list(UUID question,UUID author,boolean includeRemoved,String scope,int page,int size){return jdbc.query(SELECT+" AND (?::uuid IS NULL OR a.question_id=?::uuid) AND (?::uuid IS NULL OR a.author_id=?::uuid) AND (? OR (a.deleted_at IS NULL AND a.moderated_at IS NULL)) AND (?::varchar IS NULL OR q.scope=?) ORDER BY a.published_at DESC,a.id LIMIT ? OFFSET ?",this::map,question,question,author,author,includeRemoved,scope,scope,size,page*size);}
 public long count(UUID question,UUID author,boolean includeRemoved,String scope){return jdbc.queryForObject("SELECT count(*) "+FROM+" AND (?::uuid IS NULL OR a.question_id=?::uuid) AND (?::uuid IS NULL OR a.author_id=?::uuid) AND (? OR (a.deleted_at IS NULL AND a.moderated_at IS NULL)) AND (?::varchar IS NULL OR q.scope=?)",Long.class,question,question,author,author,includeRemoved,scope,scope);}
