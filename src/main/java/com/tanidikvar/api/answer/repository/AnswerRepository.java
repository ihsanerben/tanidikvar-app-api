...
public class AnswerRepository {
    ...
    private static final String FROM="""
        ...
        """;
    private static final String SELECT="SELECT a.*,f.id avatar_file_id,nullif(concat_ws(' ',p.first_name,p.last_name),'') author_name "+FROM;
    ...
    private Answer map(ResultSet r,int n)throws SQLException {
        return new Answer(r.getObject("id",UUID.class),r.getObject("question_id",UUID.class),r.getObject("author_id",UUID.class),r.getString("author_name"),r.getObject("avatar_file_id",UUID.class),r.getString("body"),
    ...
    public List<OwnAnswerEntry> listMine(UUID actor,int page,int size) {
        return jdbc.query("SELECT a.*,f.id avatar_file_id,q.title question_title,nullif(concat_ws(' ',p.first_name,p.last_name),'') author_name "+FROM+
