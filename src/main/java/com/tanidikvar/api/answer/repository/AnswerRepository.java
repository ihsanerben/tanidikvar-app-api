...
public class AnswerRepository {
    ...
    private static final String FROM="""
        ...
        """;
    private static final String SELECT="SELECT a.*,f.id avatar_file_id,p.education_status,nullif(concat_ws(' ',p.first_name,p.last_name),'') author_name,(SELECT count(*) FROM answer_likes l WHERE l.answer_id=a.id AND l.deleted_at IS NULL) like_count "+FROM;
    ...
    public List<OwnAnswerEntry> listMine(UUID actor,String scope,int page,int size) {
        return jdbc.query("SELECT a.*,f.id avatar_file_id,p.education_status,q.title question_title,nullif(concat_ws(' ',p.first_name,p.last_name),'') author_name,(SELECT count(*) FROM answer_likes l WHERE l.answer_id=a.id AND l.deleted_at IS NULL) like_count "+FROM+
