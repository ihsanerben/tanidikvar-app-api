...
public class AnswerRepository {
    ...
    private Answer map(ResultSet r,int n)throws SQLException {
        return new Answer(r.getObject("id",UUID.class),r.getObject("question_id",UUID.class),r.getObject("author_id",UUID.class),r.getString("author_name"),r.getString("body"),
                time(r,"published_at"),time(r,"edited_at"),time(r,"deleted_at"),r.getLong("version"));
    }
    ...
    public List<Answer> list(UUID question,int page,int size) {
        return jdbc.query(SELECT+" WHERE a.question_id=? AND a.answer_kind='COMMUNITY' AND a.deleted_at IS NULL ORDER BY a.published_at,a.id LIMIT ? OFFSET ?",this::map,question,size,page*size);
    }
    public long count(UUID question) { return jdbc.queryForObject("SELECT count(*) FROM answers a JOIN questions q ON q.id=a.question_id AND q.deleted_at IS NULL WHERE a.question_id=? AND a.answer_kind='COMMUNITY' AND a.deleted_at IS NULL",Long.class,question); }
