...
public class ApplicationRepository {
 ...
 private static final String SELECT="SELECT a.*, (u.authority='TANIDIK' AND u.active_verification_application_id=a.id) AS active_verification FROM admin_applications a JOIN users u ON u.id=a.applicant_id WHERE a.deleted_at IS NULL AND u.deleted_at IS NULL";
 private final RowMapper<AdminApplication> row=(r,n)->new AdminApplication(r.getObject("id",UUID.class),r.getObject("applicant_id",UUID.class),r.getObject("request_id",UUID.class),r.getString("submitted_first_name"),r.getString("submitted_last_name"),r.getString("education_status"),r.getObject("university_department_id",UUID.class),r.getString("university_name"),r.getString("department_name"),(Integer)r.getObject("graduation_year"),r.getString("occupation"),r.getString("company"),r.getString("cover_letter"),r.getObject("document_file_id",UUID.class),r.getString("document_sha256"),r.getLong("profile_version"),r.getString("status"),r.getTimestamp("submitted_at").toInstant(),r.getObject("reviewed_by",UUID.class),r.getTimestamp("reviewed_at")==null?null:r.getTimestamp("reviewed_at").toInstant(),r.getString("rejection_reason"),r.getLong("version"),r.getBoolean("active_verification"));
 ...
 public void insert(UUID id,UUID owner,UUID request,ProfileResponse p,String coverLetter,UUID file,String hash){
  jdbc.update("INSERT INTO admin_applications(id,applicant_id,request_id,submitted_first_name,submitted_last_name,education_status,university_id,department_id,university_department_id,university_name,department_name,graduation_year,occupation,company,cover_letter,document_file_id,document_sha256,profile_version) VALUES (?,?,?,?,?,?,?,?,NULL,?,?,?,?,?,?,?,?,?)",id,owner,request,p.firstName(),p.lastName(),p.educationStatus().name(),p.education()==null?null:p.education().universityId(),p.education()==null?null:p.education().departmentId(),p.education()==null?null:p.education().universityName(),p.education()==null?null:p.education().departmentName(),p.graduationYear(),p.occupation(),p.company(),coverLetter,file,hash,p.version());
 }
