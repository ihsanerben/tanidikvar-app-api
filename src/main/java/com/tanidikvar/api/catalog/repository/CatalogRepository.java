...
public class CatalogRepository {
    ...
    public List<CatalogEntry> list(CatalogKind kind,String query,boolean includeDeleted,int page,int size) {
        return jdbc.query("SELECT id,name,deleted_at,version FROM "+kind.table()+" WHERE (? OR deleted_at IS NULL) AND strpos(normalized_name,?)>0 ORDER BY normalized_name,id LIMIT ? OFFSET ?",
                this::entry,includeDeleted,query,size,page*size);
    }
    public long count(CatalogKind kind,String query,boolean includeDeleted) {
        return jdbc.queryForObject("SELECT count(*) FROM "+kind.table()+" WHERE (? OR deleted_at IS NULL) AND strpos(normalized_name,?)>0",Long.class,includeDeleted,query);
    }
    ...
    public List<EducationResponse> educationList(UUID university,String query,boolean includeDeleted,int page,int size) {
        return jdbc.query(EDUCATION+"WHERE ud.university_id=? AND (? OR (ud.deleted_at IS NULL AND u.deleted_at IS NULL AND d.deleted_at IS NULL)) AND strpos(d.normalized_name,?)>0 ORDER BY d.normalized_name,ud.id LIMIT ? OFFSET ?",
    ...
    public long educationCount(UUID university,String query,boolean includeDeleted) {
        return jdbc.queryForObject("SELECT count(*) FROM university_departments ud JOIN universities u ON u.id=ud.university_id JOIN departments d ON d.id=ud.department_id WHERE ud.university_id=? AND (? OR (ud.deleted_at IS NULL AND u.deleted_at IS NULL AND d.deleted_at IS NULL)) AND strpos(d.normalized_name,?)>0",Long.class,university,includeDeleted,query);
    }
