...
public class CatalogRepository {
    ...
    private static final String UNIVERSITY_COLUMNS="u.id,u.name,u.city,u.institution_type,u.description,u.website_url,u.logo_url,u.accent_primary,u.accent_soft,u.accent_foreground,(SELECT count(*) FROM programs p WHERE p.university_id=u.id AND p.deleted_at IS NULL) program_count,(SELECT count(*) FROM questions q WHERE q.university_id=u.id AND q.deleted_at IS NULL) question_count,(SELECT count(*) FROM user_profiles up JOIN users account ON account.id=up.user_id AND account.authority='TANIDIK' AND account.deleted_at IS NULL WHERE up.university_id=u.id AND up.deleted_at IS NULL) tanidik_count,u.deleted_at,u.version";
