...
public class ProgramCatalogService {
    ...
    @Transactional(readOnly=true) public CatalogOverviewResponse overview(){return new CatalogOverviewResponse(
            ...
            repository.yearly(null),
            repository.distribution("SELECT pf.name,sum(coalesce(s.placed,0)) FROM programs p JOIN program_families pf ON pf.id=p.program_family_id JOIN admission_options ao ON ao.program_id=p.id AND ao.deleted_at IS NULL JOIN admission_statistics s ON s.admission_option_id=ao.id AND s.guide_year=2026 WHERE p.deleted_at IS NULL AND pf.degree_level='LISANS' GROUP BY pf.name ORDER BY sum(coalesce(s.placed,0)) DESC,pf.name LIMIT 10"),
            repository.distribution("SELECT pf.name,sum(coalesce(s.placed,0)) FROM programs p JOIN program_families pf ON pf.id=p.program_family_id JOIN admission_options ao ON ao.program_id=p.id AND ao.deleted_at IS NULL JOIN admission_statistics s ON s.admission_option_id=ao.id AND s.guide_year=2026 WHERE p.deleted_at IS NULL AND pf.degree_level='ONLISANS' GROUP BY pf.name ORDER BY sum(coalesce(s.placed,0)) DESC,pf.name LIMIT 10"),
            repository.distribution("SELECT u.name,sum(coalesce(s.placed,0)) FROM universities u JOIN programs p ON p.university_id=u.id AND p.deleted_at IS NULL JOIN admission_options ao ON ao.program_id=p.id AND ao.deleted_at IS NULL JOIN admission_statistics s ON s.admission_option_id=ao.id AND s.guide_year=2026 WHERE u.deleted_at IS NULL GROUP BY u.id,u.name ORDER BY sum(coalesce(s.placed,0)) DESC,u.name LIMIT 10"),
            repository.distribution("SELECT u.name,count(DISTINCT p.id) FROM universities u JOIN programs p ON p.university_id=u.id AND p.deleted_at IS NULL WHERE u.deleted_at IS NULL GROUP BY u.id,u.name ORDER BY count(DISTINCT p.id) DESC,u.name LIMIT 10"),
            repository.distribution("SELECT CASE WHEN success_rank<=10 THEN 'İlk 10' WHEN success_rank<=100 THEN 'İlk 100' WHEN success_rank<=1000 THEN 'İlk 1.000' WHEN success_rank<=10000 THEN 'İlk 10.000' WHEN success_rank<=20000 THEN 'İlk 20.000' WHEN success_rank<=30000 THEN 'İlk 30.000' ELSE '30.000 sonrası' END,count(DISTINCT admission_option_id) FROM admission_statistics WHERE guide_year=2026 AND success_rank IS NOT NULL GROUP BY 1 ORDER BY min(success_rank)"),
            repository.lastSync());}
