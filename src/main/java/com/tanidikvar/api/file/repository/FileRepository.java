...
public class FileRepository {
 ...
 public Optional<UUID> avatar(UUID owner){return jdbc.query("SELECT f.id FROM stored_files f LEFT JOIN user_profiles p ON p.user_id=f.owner_id JOIN users u ON u.id=f.owner_id AND u.deleted_at IS NULL WHERE (p.user_id IS NULL OR p.deleted_at IS NULL OR u.authority='MANAGER') AND f.owner_id=? AND f.purpose='AVATAR' AND f.upload_status='READY' AND f.deleted_at IS NULL",(r,n)->r.getObject("id",UUID.class),owner).stream().findFirst();}
 public Optional<UUID> avatarForProfileCompletion(UUID owner){return jdbc.query("SELECT f.id FROM stored_files f JOIN users u ON u.id=f.owner_id AND u.deleted_at IS NULL WHERE f.owner_id=? AND f.purpose='AVATAR' AND f.upload_status='READY' AND f.deleted_at IS NULL",(r,n)->r.getObject("id",UUID.class),owner).stream().findFirst();}
}
