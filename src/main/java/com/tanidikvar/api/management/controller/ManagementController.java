...
public class ManagementController {
 ...
 @GetMapping("/actions") public PageResponse<ManagementActionResponse> actions(@AuthenticationPrincipal SessionPrincipal p,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){return management.actions(p.userId(),page,size);}
 @GetMapping("/users") public PageResponse<ManagedUserResponse> users(@AuthenticationPrincipal SessionPrincipal p,@RequestParam(required=false) String q,@RequestParam(defaultValue="ALL") String status,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){return management.users(p.userId(),q,status,page,size);}
