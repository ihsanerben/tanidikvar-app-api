...
public class ManagementController {
 ...
 @GetMapping("/content-reports") public PageResponse<com.tanidikvar.api.report.dto.ManagedContentReportResponse> contentReports(@AuthenticationPrincipal SessionPrincipal p,@RequestParam(defaultValue="") String q,@RequestParam(defaultValue="") String status,@RequestParam(defaultValue="NEWEST") String sort,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){return reports.listContent(p.userId(),q,status,sort,page,size);}
 @PutMapping("/content-reports/{id}") public com.tanidikvar.api.report.dto.ManagedContentReportResponse decideContentReport(@AuthenticationPrincipal SessionPrincipal p,@PathVariable UUID id,@Valid @RequestBody com.tanidikvar.api.report.dto.QuestionReportDecision request){return reports.decideContent(p.userId(),id,request);}
