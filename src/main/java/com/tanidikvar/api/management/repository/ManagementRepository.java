...
public class ManagementRepository {
 ...
 private static final String CONTENT="""
  ...
  """;
 private static final String CONTENT_SELECT="SELECT c.*,CASE WHEN p.user_id IS NULL THEN NULL ELSE c.author_id END author_id,coalesce(nullif(concat_ws(' ',p.first_name,p.last_name),''),'Katılımcı') author_name,(SELECT count(*) FROM question_views v WHERE v.question_id=c.question_id AND v.deleted_at IS NULL) view_count,(SELECT count(*) FROM question_likes l WHERE l.question_id=c.question_id AND l.deleted_at IS NULL) like_count,(SELECT count(*) FROM answers a WHERE a.question_id=c.question_id AND a.answer_kind='COMMUNITY' AND a.deleted_at IS NULL AND a.moderated_at IS NULL) community_answer_count,(SELECT count(*) FROM answers a WHERE a.question_id=c.question_id AND a.answer_kind='ADMIN' AND a.deleted_at IS NULL AND a.moderated_at IS NULL) admin_answer_count "+CONTENT;
 private ManagedContent content(ResultSet r,int n)throws SQLException{return new ManagedContent(r.getObject("id",UUID.class),r.getString("kind"),r.getObject("question_id",UUID.class),r.getObject("author_id",UUID.class),r.getString("title"),r.getString("body"),r.getString("author_name"),time(r,"created_at"),time(r,"deleted_at"),time(r,"moderated_at"),time(r,"archived_at"),r.getBoolean("question_hidden"),r.getLong("view_count"),r.getLong("like_count"),r.getLong("community_answer_count"),r.getLong("admin_answer_count"),r.getLong("version"));}
 ...
 SELECT p.first_name,p.last_name,un.name university_name,d.name department_name,p.graduation_year,f.id avatar_file_id,p.biography,p.occupation,p.company,p.linkedin_url,p.portfolio_url,u.active_verification_application_id,
 ...
 """,(r,n)->new ManagementUserDetail(user,r.getString("first_name"),r.getString("last_name"),r.getString("university_name"),r.getString("department_name"),(Integer)r.getObject("graduation_year"),r.getObject("avatar_file_id",UUID.class),r.getString("biography"),r.getString("occupation"),r.getString("company"),r.getString("linkedin_url"),r.getString("portfolio_url"),r.getObject("active_verification_application_id",UUID.class),r.getLong("questions"),r.getLong("community_answers"),r.getLong("admin_answers")),id));}
 ...
 public boolean editUser(UUID id,String first,String last,String biography,String occupation,String company,String linkedin,String portfolio){return jdbc.update("UPDATE user_profiles SET first_name=?,last_name=?,biography=?,occupation=?,company=?,linkedin_url=?,portfolio_url=?,updated_at=clock_timestamp(),version=version+1 WHERE user_id=? AND deleted_at IS NULL",first,last,biography,occupation,company,linkedin,portfolio,id)>0;}
