...
public class ManagementRepository {
 ...
 public List<ManagedUser> users(String query,String status,String authority,String educationStatus,int page,int size){return jdbc.query(USER_SELECT+userFilter(status)+" AND (?='' OR u.authority=?) AND (?='' OR p.education_status=?) ORDER BY u.created_at DESC,u.id LIMIT ? OFFSET ?",this::mapUser,query,authority,authority,educationStatus,educationStatus,size,page*size);}
 ...
 SELECT un.name university_name,d.name department_name,p.graduation_year,f.id avatar_file_id,p.biography,p.occupation,p.company,p.linkedin_url,p.portfolio_url,u.active_verification_application_id,
