...
public class ManagementService {
 ...
 private String kind(String kind){if(!Set.of("QUESTION","COMMUNITY","TANIDIK").contains(kind))throw new DomainException(400,"INVALID_REQUEST","İçerik türünü kontrol et.");return kind;}
 ...
 public PageResponse<ManagedUserResponse> users(UUID actor,String q,String status,String authority,String educationStatus,String sort,int page,int size){reader(actor);SearchQuery.page(page,size);String query=SearchQuery.clean(q);status(status);if(!Set.of("CREATED_DESC","CREATED_ASC","LAST_LOGIN_DESC","LAST_LOGIN_ASC").contains(sort))throw new DomainException(400,"INVALID_REQUEST","Sıralamayı kontrol et.");if(!Set.of("","MEMBER","TANIDIK","MANAGER").contains(authority))throw new DomainException(400,"INVALID_REQUEST","Yetki filtresini kontrol et.");if(!Set.of("","YKS_ADAYI","UNIVERSITE_OGRENCISI","MEZUN").contains(educationStatus))throw new DomainException(400,"INVALID_REQUEST","Rol filtresini kontrol et.");return new PageResponse<>(repository.users(query,status,authority,educationStatus,sort,page,size).stream().map(mapper::user).toList(),page,size,repository.userCount(query,status,authority,educationStatus));}
 @Transactional(readOnly=true,isolation=Isolation.REPEATABLE_READ)
