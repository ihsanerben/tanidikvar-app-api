...
public class ManagementService {
 ...
 public PageResponse<ManagedUserResponse> users(UUID actor,String q,String status,int page,int size){reader(actor);SearchQuery.page(page,size);String query=SearchQuery.clean(q);status(status);return new PageResponse<>(repository.users(query,status,page,size).stream().map(mapper::user).toList(),page,size,repository.userCount(query,status));}
 @Transactional(readOnly=true,isolation=Isolation.REPEATABLE_READ)
