...
public class ManagerWorkspaceService {
 ...
 public ManagedUserDetailResponse user(UUID actor,UUID id){reader(actor);var d=repository.detail(id).orElseThrow(this::missing);return new ManagedUserDetailResponse(mapper.user(d.user()),d.firstName(),d.lastName(),d.universityName(),d.departmentName(),d.graduationYear(),d.avatarFileId(),d.biography(),d.occupation(),d.company(),d.linkedinUrl(),d.portfolioUrl(),d.verificationId(),d.questions(),d.communityAnswers(),d.adminAnswers());}
 @Transactional public ManagedUserDetailResponse editUser(UUID actor,UUID id,ManagedUserEditRequest request){manager(actor);String first=request.firstName().strip(),last=request.lastName().strip();if(!repository.editUser(id,first,last,clean(request.biography()),clean(request.occupation()),clean(request.company()),clean(request.linkedinUrl()),clean(request.portfolioUrl())))throw new DomainException(409,"PROFILE_REQUIRED","Kullanıcının düzenlenebilir bir profili yok.");repository.audit(actor,"EDIT_USER","USER",id,"Kullanıcı profil bilgileri güncellendi.");return user(actor,id);}
