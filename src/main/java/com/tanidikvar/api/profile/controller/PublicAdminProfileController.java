...
public class PublicAdminProfileController {
 ...
 public PageResponse<PublicAdminProfileResponse> search(@RequestParam(required=false) String q,@RequestParam(defaultValue="false") boolean activeOnly,@RequestParam(required=false) UUID universityId,@RequestParam(required=false) UUID departmentId,@RequestParam(required=false) String educationStatus,@RequestParam(defaultValue="0") int page,@RequestParam(defaultValue="20") int size){return profiles.search(q,activeOnly,universityId,departmentId,educationStatus,page,size);}
