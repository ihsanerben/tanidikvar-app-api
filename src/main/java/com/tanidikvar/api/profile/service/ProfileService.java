...
public class ProfileService {
    ...
    private final PilotMode pilot;
    private final Clock clock;
    public ProfileService(ProfileRepository profiles,ProfileMapper mapper,CatalogService catalog,AccountAccessService accounts,FileRepository files,PilotMode pilot,Clock clock) {
        this.profiles=profiles; this.mapper=mapper; this.catalog=catalog; this.accounts=accounts; this.files=files; this.pilot=pilot; this.clock=clock;
    }
    ...
    public ProfileResponse update(UUID id,ProfileRequest request) {
        ...
        if(!pilot.enabled() && files.avatarForProfileCompletion(id).isEmpty()) fields.put("avatarFileId","Profilini tamamlamak için profil fotoğrafı ekle.");
