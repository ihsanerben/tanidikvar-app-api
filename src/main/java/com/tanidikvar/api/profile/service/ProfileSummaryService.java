...
public class ProfileSummaryService {
    ...
    public ProfileSummaryService(ProfileRepository profiles,FileRepository files,PilotMode pilot) { this.profiles=profiles; this.files=files; this.pilot=pilot; }
    ...
    @Transactional(readOnly=true)
    public boolean completed(UUID id) { return status(id)!=null && (pilot.enabled() || files.avatar(id).isPresent()); }
}
